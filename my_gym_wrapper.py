...
#   if obs.ndim == 1: #i.e., the original obs value was dimensionless because it was only a scalar
...
class GymWrapper(dm_env.Environment):
  ...
  #   """Computes the observation spec for the pixel observations. This overrides the base class which assumes pixels
  #
  #   Returns:
  ...
  #     spec_type = np.uint8
  #
  #   spec_name = "discrete_to_float_obs"
  #   obs_space = self._environment.observation_space
  #   spec_shape = (obs_space.n,) #This should not be hardcoded - though it might be 128 for all games, who knows
  #   #It's also not entirely clear what the frame stacker does at this point;
  #   # e.g. where is it accumulating frames?
  #   pixel_spec = specs.Array(
  #       shape=spec_shape, dtype=spec_type, name=spec_name)
  ...
  def reset(self) -> dm_env.TimeStep:
    ...
    # obsSpace = self._observation_spec
