...
from typing import NamedTuple, Optional, Sequence, Callable, Any
...
class VPGNetworks:
    """Network and pure functions for the VPG agent.
    If 'network' returns tfd.Distribution, you can use make_vpg_networks() to
...
def make_mvn_diag_ppo_networks(
    network: networks_lib.FeedForwardNetwork) -> VPGNetworks:
...
def make_discrete_networks(
  ...
  dummy_obs = jnp.array(dummy_obs, dtype=jnp.float32)
  network = networks_lib.FeedForwardNetwork(
...
def make_continuous_networks(
    ...
    def forward_fn(inputs: networks_lib.Observation):
        ...
        def policy_network(obs: networks_lib.Observation):
