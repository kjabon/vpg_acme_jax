"""Learner for the vpg agent."""

from typing import Dict, Iterator, List, NamedTuple, Optional, Tuple

import acme
from acme import types
from vpg import networks
...
class TrainingState(NamedTuple):
  """Training state for the VPG learner."""
...
class VPGLearner(acme.Learner):
  """Learner for VPG."""

  def __init__(
      self,
      vpg_networks: networks.VPGNetworks,
    ...
    def epoch_update(
      ...
      key, sub_key = jax.random.split(state.random_key)
      # (kamyar) For effiency could use same permutation for all epochs
    ...
    vmapped_network_apply = jax.vmap(
        vpg_networks.network.apply, in_axes=(None, 0), out_axes=0)
    ...
    def single_device_update(
      ...
      _, behavior_values = vmapped_network_apply(state.params,
                                                 observations)
      ...
      observations, actions, behavior_values, behavior_log_probs = jax.tree_util.tree_map(
          lambda x: x[:, :-1],
          (observations, actions,behavior_values, behavior_log_probs))
    ...
    def make_initial_state(key: networks_lib.PRNGKey) -> TrainingState:
      ...
      initial_params = vpg_networks.network.init(key_init)
  ...
  def restore(self, state: TrainingState):
    # (kamyar) Should the random_key come from self._state instead?
